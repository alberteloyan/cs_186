...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // join.
            return (double) card1 * card2 + cost1 + card1 * cost2;
        }
    }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if(joinOp.equals(Op.EQUALS)){
        	...
        	//went with .30 as described in the proj3 spec
    ...
    public Vector<LogicalJoinNode> orderJoins(
		...
		//4.       bestPlan	 = {}  // We want to find the best plan for this concrete subset 
				...
				bestPlan.cost = Double.MAX_VALUE; // initialize with the most expensive cost possible
				CostCard currPlan = new CostCard();
		...
		//6.            subplan = optjoin(s')  // Look-up in the cache the best query plan for s but with one relation missing
					currPlan = this.computeCostAndCardOfSubplan(stats, filterSelectivities, node, subset, bestPlan.cost, plans);
		...
		//9.               bestPlan = plan // Update the best plan for computing s
					if (currPlan != null) {
						if (currPlan.cost < bestPlan.cost) {
							bestPlan = currPlan;
						}
						plans.addPlan(subset, bestPlan.cost, bestPlan.card, bestPlan.plan);
					}
		...
		Set<LogicalJoinNode> joinHash = new HashSet<LogicalJoinNode>();
		joinHash.addAll(joins);
		//11. return optjoin(j)
		
        return plans.getOrder(joinHash);
    }
